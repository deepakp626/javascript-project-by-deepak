...
const showPara = ()=>{
    ...
    if(inputEl.value > text.length || inputEl.value <= 0 || inputEl.value == ""){
        alert(`Enter mininum= 1 or maximum= ${text.length}  value`)
        paragraphEl.innerHTML += ` <p class="item">${text[0]} </p><br>`
        return;
    }
    for (let index = 0; index < text.length; index++) {
        if(index == +inputEl.value){
            console.log(index , inputEl.value)
            break;
        } 
        console.log(index)
        let randomIndex = Math.floor(Math.random() * text.length)
        paragraphEl.innerHTML += `${index+1}. <p class="item">${text[randomIndex]} </p> <hr><br> `
    }
}
btnEl.addEventListener('click',showPara)
